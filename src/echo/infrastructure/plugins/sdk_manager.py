...
class SDKPluginManager(Loggable):
    ...
    def __init__(self, plugins_dirs: Union[str, List[str]], llm_factory: LLMModelFactory):
        ...
        if not SDK_AVAILABLE:
            raise RuntimeError("echo-sdk is not available. Please install it: pip install echo-sdk")
    ...
    @staticmethod
    def _is_plugin_directory(plugin_path: Path) -> bool:
        """Check if a path is a plugin directory."""
        return plugin_path.is_dir() and (plugin_path / "__init__.py").exists()
